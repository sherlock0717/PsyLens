# B 站离线规则基线标签提案报告（BILI_RULE_BASED_PROPOSAL_REPORT）

> 对 `data/v2/bili_evidence_queue.csv` 的 279 个候选单元，由**本地关键词规则基线**依据 codebook 以确定性方式生成初步提案（`tools/build_review_infra.py`）。
> 规则基线**不是模型/人工 Agent 的语义判断**，也不是人工复核结果：全部提案 `proposer_type=rule_based_baseline`、`proposal_status=rule_based_proposed_unreviewed`、`needs_human_review=true`。
...
> B 站样本高度集中在 `balance`（英雄/装备/数值平衡）与 `matchmaking`（匹配/队友）。空值主要落在 `include=no` 与部分 `uncertain`。
...
- `uncertain` 比例（机制层，占全部 279）：**54.5%**。
- 机制标签**无超出 codebook**（校验 0 违规）。

## 4. needs_human_review 与置信度

- `needs_human_review = true`：**279（100%）** —— 所有规则基线提案默认需人工复核。
...
- 本报告是**离线规则基线提案的统计**，既不是模型语义判断，也不是人工复核结论；
- 任何标签在人工于 `human_review_log.csv` 记录前，均为 `rule_based_proposed_unreviewed`；
