...
> 统一定义证据、提案、洞察、建议在流水线中的复核状态。关键原则：**离线规则基线提案不等于人工或模型语义复核**，任何未经真实人工确认的标签都不得表述为「已人工复核 / 已验证 / 可直接采信」。
...
| `rule_based_proposed_unreviewed` | 本地关键词规则基线依据 codebook 生成的提案，未经人工或模型语义复核 | 否（草稿） |
...
## 2. 提案 proposal_status（rule_based_label_proposals）

| 状态 | 含义 |
| --- | --- |
| `rule_based_proposed_unreviewed` | 离线规则基线提案，等待人工复核 |
...
| `rule_based_baseline` | 离线关键词规则基线（非人工、非模型语义判断） |
| `human` | 真实人工复核（当前不存在） |
