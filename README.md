...
PsyLens 是一个将多平台公开游戏社区反馈转化为可追溯证据链、心理机制解释与产品行动建议的分析工作流（analysis workflow）。当前公开案例以《英雄联盟》海克斯大乱斗模式为对象，整合 NGA、贴吧、B 站三个平台的公开反馈，展示从非结构化玩家表达到结构化洞察与评测的完整过程。
...
- **应用方向**：用户研究（UXR）、游戏产品 / 模式策划、社区运营与发行、AI 辅助分析工作流。
...
4. 这些反馈如何转化为产品、社区与研究可用的评测方法和行动建议？
...
- 复现需注意，公开仓库目前**缺少**完整重跑所需的：prompts、config、完整原始抓取数据、私有环境变量。
...
- 部分清洗、分类、洞察收束和行动建议**包含 AI 辅助生成**。
- AI 产物**不应被理解为完全由研究者主导的研究结论**。。
...
- AI 辅助分类**存在误差**，需人工复核。
- 行动建议是基于公开反馈形成的**产品假设**，**不代表 Riot 或腾讯等企业的内部判断**。
